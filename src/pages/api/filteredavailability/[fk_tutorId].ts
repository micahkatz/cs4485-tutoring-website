...
import { PrismaClient, Prisma, tutor, availability, appointment } from '@prisma/client';
...
const FilterAvailability = (year: number, month: number, allAvails: availability[], allAppoints: appointment[]) => {
        ...
        for( let j = 0; j < allAvails.length; j++ ) {
            ...
            if(repeat) {
                ...
                if( dayOfTheWeekAvail == dayOfTheWeekFiltered ) {
                    // Set timeframe
                    timeframe = {
                        startDT: new Date(year, month, i, allAvails[j].startDT.getHours(), allAvails[j].startDT.getMinutes()),
                        endDT: new Date(year, month, i, allAvails[j].endDT.getHours(), allAvails[j].endDT.getMinutes())
                    }
                }
            }
            else {
    ...
    for( let i = 0; i < allAppoints.length; i++ ) {
        ...
        for( let j = 0; j < timeframes.length; j++ ) {
            ...
            if( availStartHour == appointStartHour ) {
                ...
                if(timeframe.startDT.getHours() == timeframe.endDT.getHours()) {
    ...
    switch (req.method) {
        case 'GET':
            ...
            try {
                ...
                let filteredAvailability = FilterAvailability(year, month, allAvails, allAppoints)
